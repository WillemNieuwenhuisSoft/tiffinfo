...
uses
    Winapi.Windows, System.Character, System.SysUtils, System.Variants,
    System.Classes,
    ReadTiff;
...
begin
    ...
    begin
        // Address
        all.Append(StringReplace(Format('%4x:', [pos]), ' ', '0',
          [rfReplaceAll]));
        all.Append(' ');
        todo := 15;
        if total - pos < 15 then
            todo := total - pos;
        s := '';
        for l := 0 to todo do
        begin
            ...
            all.Append(StringReplace(Format('%2x', [integer(bytes[pos + l])]),
              ' ', '0', [rfReplaceAll]));
            all.Append(' ');
            if l = 7 then
                all.Append('    ');
