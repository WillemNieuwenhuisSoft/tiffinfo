...
procedure TformTiffInfo.FormClose(Sender: TObject;
...
begin
  ...
  with LB_ImageDetails do begin
    ...
    with tip do begin
      if tip.fIsBigTiff then Items.Add('Big TIFF')
      else Items.Add('Normal TIFF');
      if tip.fPalette then
        s := 'colors'
      else
        s := 'values';
      if iPhoto = 2 then iNrColors := 1 shl 24;
      if iNrColors = 0 then
        if iComp = 2 then iNrColors := 2;
      Items.Add(Format('Image size: %d x %d, %.0n %s', [iWidth, iHeight, 1.0*iNrColors, s]));
      case iPhoto of
        0 : Items.Add('WhiteIsZero: Colors range from white to black');
        1 : Items.Add('BlackIsZero: Colors range from black to white');
        2 : Items.Add('RGB: Full color image (' + IntToStr(iNrBits) + ' bits per pixel)');
        3 : Items.Add('Colors are determined from a palette (LUT)');
        4 : Items.Add('Image is a bit mask');
      end;
      if (sResUnit = 'Inch') or (sResUnit = 'Centimeter') then begin
        if iXRes <> 0 then
        ...
        if sResUnit <>'' then Items.Add(sResUnit);
      case iComp of
        1     : Items.Add('No compression');
        2     : Items.Add('CCITT Group 3 1-dimensional modified HuffMan runlength encoding');
        3     : Items.Add('CCITT T.4 bi-level encoding');
        4     : Items.Add('CCITT T.6 bi-level encoding');
        5     : begin
                  s := 'LZW compression';
                  if iPredict = 1 then
        ...
        6     : Items.Add('JPEG compression');
        32773 : Items.Add('Packbits compression');
      else
...
//        Items.Add('GDAL metadata: ');
  ...
  if tip.fPalette then begin
    DG_Palette.OnDrawCell := DG_PaletteDrawCell;
    if tip.iNrColors = 16 then begin
...
//  TS_GeoTiffSummary.TabVisible := tip.fGeoTiff;
  if tip.fGeoTiff then begin
    with LV_GeoTiff do begin
      Items.Clear;
      for i := 0 to ttInfo.iNrGeoKeys - 1 do begin
        litem := Items.Add;
    ...
    with LB_GeoTiff do begin
      ...
      if abs(tgp.t3Scale.X) > 0.001 then begin
        with tgp.t3Scale do
        ...
        for i := 0 to tgp.inrTiePts - 1 do
          with tgp.ptieTiePoints^[i] do begin
            Items.Add(Format('%10s(%.2f, %.2f, %.2f) -> (%.8f, %.8f, %.8f)',
                      ['', t3Pixel.X, t3Pixel.Y, t3Pixel.Z,
                       t3World.X, t3World.Y, t3World.Z]));
          end;
      end;
      if tgp.fMatrix then begin
...
var i, j, iLineHeight : integer;
    iLeftMargin, iXOffset, iXDPI,
    iLeftIFD,      // information will be indented 1 cm
    iScreenDPI,
    iCurVert, iCurHorz : integer;
    rectClip : TRect;
begin
  if ttInfo.sTiffName = '' then exit;
  if not PD_Tiff.Execute then exit;
  Printer.BeginDoc;
...
{ print Header }
    ...
    TextOut(rectClip.Right - TextWidth(ttInfo.sTiffName) - 10, 0, ttInfo.sTiffName);
...
{ print IFD overview }
    ...
    with LV_TiffInfo.Columns do
      for i := 0 to Count - 1 do begin
        ...
        iCurHorz := iCurHorz + MulDiv(Items[i].Width, iXDPI, iScreenDPI);
      end;
    iCurVert := iCurVert + iLineHeight;
    Font.Size := formTiffInfo.Canvas.Font.Size;
    iLineHeight := MulDiv(TextHeight('Hj'), 4, 3); // line spacing = 33%
    for i := 0 to ttInfo.iNumberIFD - 1 do begin
      iCurHorz := iLeftIFD;
      with LV_TiffInfo.Items[i] do begin
        TextOut(iCurHorz, iCurVert, Caption);
        iCurHorz := iCurHorz + MulDiv(LV_TiffInfo.Columns[0].Width, iXDPI, iScreenDPI);
        for j := 0 to SubItems.Count - 1 do begin
          TextOut(iCurHorz, iCurVert, SubItems[j]);
          iCurHorz := iCurHorz + MulDiv(LV_TiffInfo.Columns[j + 1].Width, iXDPI, iScreenDPI);
        end;
        iCurVert := iCurVert + iLineHeight;
      end;
    end;
...
{ Print Image Info }
    ...
    if ttInfo.fHasGeoTiff then begin
      Font.Size := 15;
      Font.Style := [fsBold];
      Font.Name := 'Times New Roman';
      iLineHeight := MulDiv(TextHeight('Hj'), 5, 4); // line spacing = 20%
...
{ Print GeoTiff Tag Info }
      ...
      for i := 0 to ttInfo.iNrGeoKeys - 1 do begin
        iCurHorz := iLeftIFD;
        with LV_GeoTiff.Items[i] do begin
          TextOut(iCurHorz, iCurVert, Caption);
          iCurHorz := iCurHorz + MulDiv(LV_GeoTiff.Columns[0].Width, iXDPI, iScreenDPI);
          for j := 0 to SubItems.Count - 1 do begin
            TextOut(iCurHorz, iCurVert, SubItems[j]);
            iCurHorz := iCurHorz + MulDiv(LV_GeoTiff.Columns[j + 1].Width, iXDPI, iScreenDPI);
          end;
          iCurVert := iCurVert + iLineHeight;
        end;
      end;
...
procedure TformTiffInfo.DG_PaletteDrawCell(Sender: TObject; Col,
  Row: integer; Rect: TRect; State: TGridDrawState);
var tip : TImageProperties;
begin
  ttInfo.GetImageProperties(tip);
  with DG_Palette.Canvas do begin
...
var li : TListItem;
begin
  if LV_GeoTiff.SelCount > 0 then begin
    li := LV_GeoTiff.Selected;
    with F_GeoTiffKeyDetail do begin
...
begin
    ...
    if LV_TiffInfo.SelCount > 0 then with F_Detail do begin
        ...
        if li.SubItems[1] = 'Char (7-bit)' then begin
...
begin
  ...
  slReport.Add('Report for ' + ttInfo.sTiffName + ' generated: ' + FormatDateTime('ddd, d mmm yyyy, t', Now));
  slReport.Add('');
...
begin
  ...
  for i := 0 to mt.count - 1 do begin
    ts.add(IntToStr(mt.all[i].imgID) + ' ' + IntToStr(mt.all[i].subIFD) + ' ' + IntToStr(mt.all[i].IFDoffset));
  end;
